...
`,
    ...
    test("Checker.getTrueTypeOfConditionalType() and getFalseTypeOfConditionalType()", async () => {
        const api = spawnAPI(typeFiles);
        try {
            const snapshot = await api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            ...
            const trueType = await project.checker.getTrueTypeOfConditionalType(type as ConditionalType);
            ...
            const falseType = await project.checker.getFalseTypeOfConditionalType(type as ConditionalType);
