...
`,
    ...
    test("Checker.getTrueTypeOfConditionalType() and getFalseTypeOfConditionalType()", () => {
        const api = spawnAPI(typeFiles);
        try {
            const snapshot = api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            ...
            const trueType = project.checker.getTrueTypeOfConditionalType(type as ConditionalType);
            ...
            const falseType = project.checker.getFalseTypeOfConditionalType(type as ConditionalType);
